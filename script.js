...
class text {
	constructor( root ) {
		this._root = root;
		this._elClips = root.querySelectorAll( ".text-clip" );
		this._elWords = root.querySelectorAll( ".text-word" );
  ...
  "†hε υηι†ε∂ σяιgιηѕ?"
	...
	_glitch() {
		...
		this._root.classList.add( "text-blended" );
	}
	_unglitch() {
		this._removeClipCSS();
		this._textContent( this._text );
		this._root.classList.remove( "text-blended" );
	}
	_textContent( txt ) {
